...
const repeatWord = word => {
   let counter = 0
   message.split(' ').forEach(m => {
      if(m.toLowerCase() === word)
         counter++;
   })
   return counter;
}
const uniqueMessage = new Set(message.toLowerCase().split(' '))

uniqueMessage.forEach(um => {
   let repeat = repeatWord(um)
   repeatWords += um + repeat
})
