...
const getChecksum = word => {
   let str = word.split('-')[0]
   let checksum = ""
   let result = ""
   str.split('').forEach(c => {
      if(getCountChar(str,c) === 1) checksum += c
   })
...
files.split('\n').forEach((word) => {
   let cs = word.split('-')[1]
   if(getChecksum(word) === cs) counterReal++
   if(counterReal === 33) result = getChecksum(word)
})
