...
const allPasswords = passwords.split('\n')
let counter = 0
let result = ""

const isValid = pass => {
   let contrasena = pass.split(' ')
   let separator = contrasena[0].indexOf('-')
   let min = parseInt(contrasena[0].substring(0, separator))
   let max = parseInt(contrasena[0].substring(separator + 1))
   let letter = contrasena[1][0]
   let psw = contrasena[2]
   let counter = 0

   psw.split('').forEach(el => {
      if (el === letter) counter++
   })
...
allPasswords.forEach(p => {
   ...
   if (counter === 42) result = p.split(' ')[2]
})
